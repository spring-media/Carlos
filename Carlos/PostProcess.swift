...
extension CacheLevel {
  ...
  */
  ...
  - returns: A transformed CacheLevel that incorporates the post-processing step
  */
  public func postProcess(transformerClosure: OutputType -> Promise<OutputType>) -> BasicCache<KeyType, OutputType> {
...
public func postProcess<A, B: OneWayTransformer where B.TypeIn == B.TypeOut>(fetchClosure: (key: A) -> Promise<B.TypeIn>, transformer: B) -> BasicCache<A, B.TypeOut> {
...
*/
@available(*, deprecated=0.5)
public func postProcess<A, B>(fetchClosure: (key: A) -> Promise<B>, transformerClosure: B -> Promise<B>) -> BasicCache<A, B> {
...
*/
@available(*, deprecated=0.5)
public func postProcess<A: CacheLevel>(cache: A, transformerClosure: A.OutputType -> Promise<A.OutputType>) -> BasicCache<A.KeyType, A.OutputType> {
...
- returns: A CacheLevel that incorporates the post-processing step
*/
public func ~>><A, B: OneWayTransformer where B.TypeIn == B.TypeOut>(fetchClosure: (key: A) -> Promise<B.TypeIn>, transformer: B) -> BasicCache<A, B.TypeOut> {
  return wrapClosureIntoFetcher(fetchClosure).postProcess(transformer)
}
...
- returns: A CacheLevel that incorporates the post-processing step
*/
public func ~>><A, B>(fetchClosure: (key: A) -> Promise<B>, transformerClosure: B -> Promise<B>) -> BasicCache<A, B> {
  return wrapClosureIntoFetcher(fetchClosure).postProcess(wrapClosureIntoOneWayTransformer(transformerClosure))
}
...
- returns: A transformed CacheLevel that incorporates the post-processing step
*/
public func ~>><A: CacheLevel>(cache: A, transformerClosure: A.OutputType -> Promise<A.OutputType>) -> BasicCache<A.KeyType, A.OutputType> {
  return cache.postProcess(wrapClosureIntoOneWayTransformer(transformerClosure))
}