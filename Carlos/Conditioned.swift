...
private func conditionedClosure<A, B>(closure: A -> Result<B>, condition: A -> Result<Bool>) -> (A -> Result<B>) {
  return { input in
    let request = Result<B>()
...
extension OneWayTransformer {
  ...
  */
  public func conditioned(condition: (TypeIn) -> Result<Bool>) -> OneWayTransformationBox<TypeIn, TypeOut> {
...
extension TwoWayTransformer {
  ...
  */
  public func conditioned(condition: (TypeIn) -> Result<Bool>, inverseCondition: (TypeOut) -> Result<Bool>) -> TwoWayTransformationBox<TypeIn, TypeOut> {
